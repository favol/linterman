 ...
 * ```
 */
export async function lint(collection, config = {}) {
 ...
 */
export function lintSync(collection, config = {}) {
    if (!wasmModule) {
        throw new Error('WASM module not initialized. Call initWasm() first.');
    }
    const validatedConfig = LintConfigSchema.parse({
 ...
 * ```
 */
export async function lintAndFix(collection, config = {}) {
    // Initialiser WASM si nécessaire
    if (!wasmModule) {
        await initWasm();
    }
 ...
 */
export function lintAndFixSync(collection, config = {}) {
    if (!wasmModule) {
        throw new Error('WASM module not initialized. Call initWasm() first.');
    }
    const validatedConfig = LintConfigSchema.parse({
